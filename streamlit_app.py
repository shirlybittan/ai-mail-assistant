...
import pandas as pd
import os
import time # Make sure 'time' is imported for time.sleep()
import datetime
...
# This ensures that when the app reruns (e.g., after sending emails and keys are deleted),
# they get their default values back.
...
def log_message(message: str, is_error: bool = False):
    ...
    try:
        ...
        with open(LOG_FILE_PATH, "a", encoding="utf-8") as f:
            f.write(log_entry + "\n")
            f.flush() # <--- ADDED: Forces the log to be written immediately to file
...
if st.session_state.awaiting_confirmation:
    with col2_send_conf:
        ...
        if st.button("Confirm Send All Emails", key="confirm_send_final_button"):
            ...
            print("CONSOLE LOG (DIRECT): Starting email sending process now!") # <--- ADDED: Direct print
            ...
            if not st.session_state.contacts:
                ...
                # Force a small delay here to give logs a chance to flush before rerun
                time.sleep(0.5) # <--- ADDED: Small delay to help logs flush
                ...
                # These are the ones tied to the main input widgets:
