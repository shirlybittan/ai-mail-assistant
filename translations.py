...
TRANSLATIONS = {
    ...
    "Personalize each email (uses contact name and makes it unique)": {"en": "Personalize each email (uses contact name and makes it unique)", "fr": "Personaliser chaque e-mail (utilise le nom du contact et le rend unique)"},
    ...
    "TEMPLATE-BASED": {"en": "BASÉ SUR UN MODÈLE", "fr": "BASÉ SUR UN MODÈLE"},
...
def _t(key):
    ...
    lang = st.session_state.get("language", "en") # Default to English if not set
